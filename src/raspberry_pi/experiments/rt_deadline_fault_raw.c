...
/*
 * CONTROL overload fault injection.
 *
 * Between 4 and 6 seconds, the highest-priority CONTROL service
 * deliberately consumes 25 ms of CPU time even though its period
 * and deadline are only 20 ms.
 */
...
/*
 * Return elapsed time since the common real-time experiment start.
 */
static int64_t experiment_elapsed_ns(void)
{
    return
        monotonic_now_ns() -
        timespec_to_ns(&g_start_time);
}


/*
 * Intentionally consume CPU for a fixed amount of wall-clock time.
 *
 * This is used only for controlled deadline-fault injection.
 * Unlike sleep(), busy waiting keeps the highest-priority CONTROL
 * thread runnable and therefore creates genuine CPU contention.
 */
...
{
    /*
     * Controlled timing-fault injection.
     *
     * CONTROL period/deadline = 20 ms
     * injected execution      = 25 ms
     *
     * Therefore the CONTROL service cannot satisfy its deadline
    ...
    if((elapsed_ns >= OVERLOAD_START_NS) &&
       (elapsed_ns < OVERLOAD_END_NS))
    {
        BusyWaitNs(
            CONTROL_OVERLOAD_NS
        );
